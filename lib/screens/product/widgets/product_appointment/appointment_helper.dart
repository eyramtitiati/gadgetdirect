...
bool activeDayAppointment(
  ...
  if(DateTime.now().day == day.day &&
    ...
    if(activeHours.where((element) => element.day == day.day &&
        element.month == day.month &&
        element.year == day.year && element.isAfter(DateTime.now())).isNotEmpty){
     return true;
    }else{
      return false;
    }
  ...
  if (durationUnit != "day") {
    return (hasRestrictedDay)
        ? !(activeHours.firstWhere(
            ...
            element.month == day.month &&
            element.year == day.year && get(restrictedDays, [day.weekday.toString()], null) != null),
...
bool isScheduledDay({
  ...
  for (DateTime time in scheduleTimes) {
    if (time.year == date.year &&
        ...
        time.day == date.day && date.isAfter(DateTime.now())) {
      return true;
    }
  }
  return false;
